...
function FavoritesPage() {
  ...
  return <Container className="py-10">
      <SectionTitle title={t('favorites.title')} subtitle={t('favorites.subtitle')} />
      ...
      {loading ? <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 text-sm text-slate-600 dark:text-slate-300">{t('loading')}</div> : favorites.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-10 text-center">
